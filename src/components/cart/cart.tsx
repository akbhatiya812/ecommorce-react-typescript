        ...
        <div className={style.cartItemsContainer}>
            {cart.map( (item,index) => (<div key={index} className={style.itemContainer}>
                ...
                <div className={style.itemPriceContainer}>
                    <p> <span className={style.icons + " " + style.plus}><CiCirclePlus onClick={() => addToCart(item)} /></span> {item.cartQty} x ₹{item.price} <span className={style.icons + " " + style.minus}><CiCircleMinus onClick={() => {minusFromCart(item)}} /> &nbsp; <CiTrash onClick={() => removeFromCart(item)} /></span> </p>
                </div>
